...
local function display(orange, red, onlyLocal)
    ...
    local lines = ''--'<u><span font_desc="Neuropol bold">diskusage</span></u>\n'
    ...
    for i, m in pairs(data) do
		...
		hdd_use[j] = blingbling.progress_graph({height = 22, width = 8, rounded_size = 0.2})
		vicious.register(hdd_use[j], vicious.widgets.fs, "${".. i .." used_p}", 120) --"$"..(i+1).."",1)
		j=j+1
  ...
  --              .. "\t"
    ...
    return hdd_use --lines
end
-- }}}
-- }}}

-- {{{ global functions
function M.addToWidget(mywidgets, orange, red, onlyLocal)
  
    local foos = display(orange, red, onlyLocal)
	mywidgets = foos[0]
 -- mywidget:set_markup( string.format('<span font_desc="%s">%s</span>', "Envy Code R 8", display(orange, red, onlyLocal)))
